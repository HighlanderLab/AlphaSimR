...
//
// Args:
// args: Common MaCS command-line arguments shared across chromosomes,
//   excluding the chromosome-specific seed suffix.
// maxSites: Maximum number of segregating sites to retain per chromosome.
//   Zero means keep all sites returned by MaCS for that chromosome.
// inbred: Whether the simulated haplotypes should be interpreted as fully
//   inbred individuals.
// ploidy: Ploidy of the returned founder population.
// nThreads: Number of OpenMP threads used across chromosomes.
// seed: Numeric seeds generated by R. MaCS uses them as explicit chromosome
//   seeds, and the same seeds are used for post-MaCS site subsampling.
//
// RNG note:
//   MaCS uses its own RNG and does not use R or alphasimrRng streams.
//   Each chromosome MaCS task receives one explicit integer seed from R.
//   That seed is converted to the MaCS command-line suffix for the simulation
//   and then reused for site subsampling after MaCS simulation.
//   This keeps the full MaCS path reproducible.
//
// Returns:
// A list with packed genotype cubes ("geno") and chromosome genetic maps
// ("genMap").
// [[Rcpp::export]]
