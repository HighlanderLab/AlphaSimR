...
runMacs = function(nInd,nChr=1, segSites=NULL, inbred=FALSE, species="GENERIC",
  ...
  if(!is.null(manualCommand)){
    ...
    if(species=="GENERIC"){
      ...
      speciesParams = paste(c(round(cattleChrBp), "-t", mutRate * 4 * Ne, "-r", recRate * 4 * Ne), collapse = " ")
      histNe = histNe/Ne
  ...
  genMap = .runMacsGenMapFromMacs(macsOut$genMap, genLen, manualCommand)
...
.runMacsGenMapFromMacs <- function(macsGenMap, genLen, manualCommand = NULL) {
  nChr <- length(macsGenMap)
  if (length(genLen) == 1L) {
    genLen <- rep(genLen, nChr)
  }
  if (length(genLen) != nChr) {
    stop("genLen must have length 1 or nChr", call. = FALSE)
  }
  ...
  for (i in seq_len(nChr)) {
    pos <- as.numeric(macsGenMap[[i]])
    if (is.null(recMap)) {
      genMap[[i]] <- genLen[[i]] * c(pos - pos[[1L]])
    } else {
      genMap[[i]] <- rateMap2cumMorgan(pos, recMap$breaks[[i]], recMap$rates[[i]])
      genMap[[i]] <- genMap[[i]] - genMap[[i]][[1L]]
    }
    names(genMap[[i]]) <- paste(i, seq_along(genMap[[i]]), sep = "_")
  }
  names(genMap) <- as.character(seq_len(nChr))
  genMap
}
